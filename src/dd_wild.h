...
struct WildPainter {
  ...
  void chest() {
    ...
    float nw2 = w * R.rr(0.2f, 0.28f);
    float ny0 = s * 0.84f, ny1 = s * R.rr(0.98f, 1.12f);
    ...
    float sw = w * R.rr(1.35f, 2.15f);
    float sy = ny1, drop = s * R.rr(0.12f, 0.3f);
    float hem = s * R.rr(1.32f, 1.55f);
    ...
    if (t.chestMode == 5) {   // poncho: one big triangle, zigzag hem
      ...
      asPen();
      Vec2 l1[2] = { { nx, sy - s * 0.03f }, { nx - sw * R.rr(0.9f, 1.1f), py2 } };
      Vec2 l2[2] = { { nx, sy - s * 0.03f }, { nx + sw * R.rr(0.9f, 1.1f), py2 } };
      mixedLine(l1, 2, pc, 0.85f * inkA, 1, &cc);
      mixedLine(l2, 2, pc, 0.85f * inkA, 1, &cc);
      P48 hm;
      for (int i = 0; i <= 7; i++)
    ...
    patch(nx, sy + s * R.rr(0.22f, 0.34f), sw * R.rr(0.5f, 0.68f), s * R.rr(0.16f, 0.28f),
          R.rr(-0.12f, 0.12f), cc, 0.6f * fillA);
    asPen();
    for (int sd = -1; sd <= 1; sd += 2) {
      float swS = sw * R.rr(0.86f, 1.14f);   // shoulders come in unequal pairs
      float dropS = drop * R.rr(0.8f, 1.2f);
      P96 sm;
      if (t.shoulderMode == 1) {        // squared: a coat-hanger corner
        Vec2 sh[3] = { { nx + sd * nw2 * 1.3f, sy },
                       { nx + sd * swS * R.rr(0.82f, 0.95f), sy + dropS * R.rr(0.05f, 0.2f) },
                       { nx + sd * swS, sy + dropS } };
        sm.clear();
        ...
        Vec2 sh[4] = { { nx + sd * nw2 * 1.35f, sy },
                       { nx + sd * swS * 0.55f, sy - dropS * R.rr(0.15f, 0.4f) },
                       { nx + sd * swS * R.rr(0.95f, 1.05f), sy + dropS * 0.45f },
                       { nx + sd * swS, sy + dropS } };
        chaikin(sm, sh, 4, false, 2);
      } else if (t.shoulderMode == 3) { // fallen: it all slides off steeply
        dropS *= R.rr(1.7f, 2.3f);
        swS *= R.rr(0.78f, 0.92f);
        Vec2 sh[3] = { { nx + sd * nw2 * 1.3f, sy },
                       { nx + sd * swS * 0.55f + R.rr(-4, 4), sy + dropS * 0.5f + R.rr(-4, 4) },
                       { nx + sd * swS, sy + dropS } };
        chaikin(sm, sh, 3, false, 1);
      } else {                          // sloped, the original
        Vec2 sh[3] = { { nx + sd * nw2 * 1.35f, sy },
                       { nx + sd * swS * 0.6f, sy + dropS * R.rr(0.45f, 0.6f) },
                       { nx + sd * swS, sy + dropS } };
        chaikin(sm, sh, 3, false, 1);
      }
      mixedLine(sm.p, sm.n, pc, 0.85f * inkA, 1, &cc);
      if (R.chance(0.8f)) {   // side seams running off the page
        Vec2 sv[2] = { sm.p[sm.n - 1], { nx + sd * swS * R.rr(0.9f, 1.06f), hem } };
        mixedLine(sv, 2, pc, 0.7f * inkA, 0.9f, &cc);
      }
    }
    if (t.chestMode == 0) {          // tee: crew neck, maybe a scribbled print
      ...
      mixedLine(sm2.p, sm2.n, pc, 0.85f * inkA);
      if (R.chance(0.6f))
